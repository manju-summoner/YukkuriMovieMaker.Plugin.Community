...
{
    internal sealed class OpenFxScanCacheSettings : SettingsBase<OpenFxScanCacheSettings>
    {
        static readonly object saveLock = new();

        public override SettingsCategory Category => SettingsCategory.VideoEffect;
        public override string Name => "OpenFX Scan Cache";
        ...
        public Dictionary<string, PersistentPluginScanCacheEntry<OpenFxScannedPluginInfo>> Entries { get; set; } = new(StringComparer.OrdinalIgnoreCase);
        ...
        public override void Initialize()
        {
            ...
            if (!ReferenceEquals(Entries.Comparer, StringComparer.OrdinalIgnoreCase))
            {
                ...
                var rebuilt = new Dictionary<string, PersistentPluginScanCacheEntry<OpenFxScannedPluginInfo>>(StringComparer.OrdinalIgnoreCase);
    ...
    internal sealed class OpenFxScanCacheSettingsStorage : IPersistentPluginScanCacheStorage<OpenFxScannedPluginInfo>
    {
        public PersistentPluginScanCacheState<OpenFxScannedPluginInfo> Load()
        {
            var settings = OpenFxScanCacheSettings.Default;
            PersistentPluginScanCacheState<OpenFxScannedPluginInfo>? state = null;
            OpenFxScanCacheSettings.WithSaveLock(() =>
            {
                state = new PersistentPluginScanCacheState<OpenFxScannedPluginInfo>
                {
        ...
        public void Save(PersistentPluginScanCacheState<OpenFxScannedPluginInfo> state)
        {
            var settings = OpenFxScanCacheSettings.Default;
            OpenFxScanCacheSettings.WithSaveLock(() =>
            {
