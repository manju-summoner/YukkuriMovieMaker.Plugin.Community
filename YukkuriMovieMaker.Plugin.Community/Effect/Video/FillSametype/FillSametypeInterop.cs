...
{
    internal sealed class FillSametypeInteropProvider : ComputeExternalDirect3D11Provider
    {
        private readonly ID2D1DeviceContext6 renderContext;

        private FillSametypeInteropProvider(
        ...
        public static FillSametypeInteropProvider? TryCreate(
            ...
            out GraphicsDevice? graphicsDevice)
        {
            ...
            try
            {
                ...
                var provider = new FillSametypeInteropProvider(device, context, renderContext, scheduler);
    ...
    internal sealed partial class FillSametypeInteropHost
    {
        ...
        private void WriteMask(
            ...
            [ComputeResource(ComputeResourceAccess.Read)] IReadOnlyBuffer<int> labels,
            [ComputeResource(ComputeResourceAccess.ReadWrite)] ReadWriteBuffer<int> matchFlags,
            int invert,
            int width,
            int height)
        {
            ...
            context.For(width, height, new SharedMaskShader(labels, matchFlags, destination, invert, width, height));
        }
        ...
        private void ExtractForeground(
            ...
            [ComputeResource(ComputeResourceAccess.ReadWrite)] ReadWriteBuffer<int> foreground,
            int width,
            int height)
        {
            _ = device;

            context.For(width, height, new ForegroundFromSharedShader(source, foreground, width, height));
        }
            ...
            int height)
        {
            ...
            context.For(width, height, new ClearSharedMaskShader(destination, width, height));
        }
    }

    [ComputeInteropResourceSet]
    internal sealed partial class FillSametypeResourceSet
    {
        ...
        private readonly SharedTextureSlot<Bgra32, Float4, ExternalDirect3D11TextureView> mask;
    }
}
